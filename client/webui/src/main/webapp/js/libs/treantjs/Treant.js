  ...
  '../../../hkt/js/ui/UIPopup',
		...
		*/
		...
		handleOverflow: function(treeWidth, treeHeight) {
			...
			if(this._R) {
				...
				this._R = new Raphael(this.drawArea, viewWidth, viewHeight);
			}
	...
	*/
	...
	TreeNode.prototype = {
		...
		addMouseEvent: function(my_switch, flag) {
			UTIL.addEvent(my_switch, 'click', function(){
				if(flag){
					my_switch.gui.load(my_switch.code);
					my_switch.viewStack.push(my_switch.gui) ;
				}else{
					my_switch.viewStack.push(my_switch.gui);
				}
			});
					...
					for(var key in this.text) {
						if(TreeNode.CONFIG.textClass[key]) {
							...
							if (this.text[key].href) {
								...
								text.gui = this.text[key].gui;
								text.code = this.text.code;
								text.name = this.text.name;
								text.viewStack = this.text.viewStack;
								this.addMouseEvent(text,true);
								if (this.text[key].target) { text.target = this.text[key].target; }
							}else{
								text.href = " ";
								text.gui = this.text.gui;
								text.viewStack = this.text.viewStack;
								this.addMouseEvent(text,false);
							}
