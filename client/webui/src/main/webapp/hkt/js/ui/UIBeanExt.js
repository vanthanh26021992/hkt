define([
  ...
  'i18n',
  'ui/Validator',
  'text!ui/UIBeanExt.jtpl',
  'css!ui/UIBeanExt.css'
  ...
  var SearchFilterTemplate = _.template(
    ...
    "  <a href='' class='onSelectCustomInput' itemIdx='<%=i%>'>" +
    "    <%=items[i].label%>" + 
    "  </a>" +
    "<%}%>"
  ) ;
   ...
   *@type ui.UIBean 
   */
  var UIBeanExt = Backbone.View.extend({
    ...
    bind: function(name, bean, setDefault) {
      ...
      this.beanStates[name] = { bean: bean, editMode: false, select: select, state: {}, isCheck: false, search: null} ;
    },
...
//    
//    load: function() {
//    	////console.log(this.config.beans.bean.fields[0].field)= "long";
//	},
    ...
    onSelectCustomInput: function(evt) {
      ...
      fieldConfig.custom.set( bean, selectObj, this, beanState) ; //Sửa phần này, kiểm tra nếu lỗi thì kiểm tra từ phần này
    	  input.val(fieldConfig.custom.getDisplay(bean)) ;
